...
def Measure(fname=None,trans_label=None):
    
    print('Measuring Temperature Image...')
...
#    m.pset('vectorfrequency', 390625) #12.5e6)
    ...
    scan_all_mea_temperature = m.Generate_scan_vector_mea_temperature()
...
#    time.sleep(0.1)
    ...
    # measure impedance
    m.UpdateScanChain(scan_all_mea_temperature)
    time.sleep(1)        
...
#    plt.title('adc 1 channel')
#    plt.show() 
#
#    plt.figure(figsize=(8,12))
#    plt.plot(adcdata[0::8][500000:500200])
#    plt.title('adc 1 channel')
#    plt.show() 
...
#            plt.show()
        ...
        data_1_ch_avg = data_1_ch_avg[:,3:-3]
...
#    
    ...
    # parameters
    T = m.pget('Temperature')
    VCM = m.pget('V_CM')
    
    
    with open('data/temperature/image_ph1_T'+str(T)+'_VCM'+str(VCM)+'_drift_'+str(trans_label)+'s.npy','wb') as f:
        np.save(f, image_2d_ph1)

    with open('data/temperature/image_ph2_T'+str(T)+'_VCM'+str(VCM)+'_drift_'+str(trans_label)+'s.npy','wb') as f:
        np.save(f, image_2d_ph2)
#
#    if fname is None:
#        fname = r"C:\Users\larki\Documents\minerva_data\F0000_xxxxxx_minerva_biofilm_xx.h5"
#        
#    if 1:
...
#               cmap='Blues')               
#    plt.title('temperature ph1 (normalized, Farads)')
#    plt.colorbar()
#    
#    plt.subplot(1,2,2)
#    plt.imshow(image_2d_ph2,
...
#               cmap='Blues')
#    plt.title('temperature ph2 (normalized, Farads)')
#    plt.colorbar()
#    plt.show() 
#
...
if __name__ == "__main__":
    ...
    #print('5 sec delay')
    drift_t_start = time.time()
    time_steps = np.zeros(180)
    for i in range(180):
        time.sleep(43)  # each run takes roughly 17s
        trans_label = int(time.time() - drift_t_start)
        time_steps[i] = trans_label
        Measure(trans_label = trans_label)
    
    print('time_steps = ',time_steps)
    with open('data/temperature/temperature_drift_overnight_time_stamp.npy','wb') as f:
        np.save(f, time_steps)
#    for loop in range(int(60/5 * 16)): # 18 hours
#        print('measure')
#        fname = r"C:\Users\larki\Documents\minerva_data\F0095_09022021_minerva_biofilm_M7.h5"
#        Measure(fname=fname)
#        print('sleeping')
#        time.sleep(60*5) #5 min delay
