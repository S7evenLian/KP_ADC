...
def Measure(fname=None, ect_pairs=[(1,1),], block=5):
...
#    m.pset('vectorfrequency', 390625) #12.5e6)
    ...
    m.pset('V_SW', 400)
    m.pset('V_CM', 400)    
    m.pset('V_Electrode_Bias', 400)
    m.pset('V_STIMU_P', 1800)
    m.pset('V_STIMU_N', 0)   
    m.pset('V_STBY', 100)
    ...
    for k,(row_offset,col_offset) in enumerate(ect_pairs):
        ...
        scan_all_mea_ECT = m.Generate_scan_vector_mea_ECT(row_offset=row_offset,
                                                  col_offset=col_offset, block=block)
    ...
    #    time.sleep(0.1)
        ...
        #m.SendRawVector(vector_reset_deselect)
        #time.sleep(0.1)
        scan_all_set_sensing_mode = m.Generate_scan_vector_all_pixel_sensing_mode(0)
        m.UpdateScanChain(scan_all_set_sensing_mode)  # send as 32 bit vector
        time.sleep(0.02)
...
#        plt.title('adcdata 1')
#        plt.subplot(1,2,2)
#        plt.plot(adcdata[80000:81000:8])
#        plt.title('adcdata 1')
#        plt.show() 
...
if __name__ == "__main__":
    ...
    fname = "data\ECT_debug\ECT_debug_18x18.h5"
    
    import itertools
    ect_pairs = list(itertools.product(range(-9,10,1), range(-9,1,1)))
    
    print('Running ECT for offsets:', ect_pairs)    
    print('Total pairs: '+str(len(ect_pairs)))
    
    # measure 10 pairs at a time
    length = len(ect_pairs)
    for i in range(int(length/10)):
        Measure(fname=fname,
                ect_pairs=ect_pairs[i*10:(i+1)*10], block=9)
    
    # send in the remaining pairs
    Measure(fname=fname,ect_pairs=ect_pairs[length-length%10:length], block=9)
