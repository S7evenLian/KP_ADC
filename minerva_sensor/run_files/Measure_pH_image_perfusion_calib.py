...
if __name__ == "__main__":
    ...
    V= list(np.arange(840,1180,40)) #mV Vgs
    # sleepduration = 5*60 #seconds
    fname = r"R:\Eng_Projects\EmbeddedBioelectronics\projects\Minerva\data\pH_calib\pH_9c_flow.h5"
    # for i in range(3):
    i = 0 #flag
    for j in range(len(V)):
        
        # if(i%len(V) == j):
        Measure(V[j],i,fname=fname)
        print("Vref is %u mV" %(V[j]))
        # if(i%3 == 1):
        #     Measure(V[1],i)
        #     print("Vref is %u mV" %(V2))
        # if(i%3 == 2):
        #     Measure(V[2],i)
        #     print("Vref is %u mV" %(V3))
            
        Standby()
        time.sleep(2)
        i=1
        # stimstart = time.time()
        # while time.time()-stimstart < sleepduration:
        #     Standby()
